...
data_long <- data_long %>% 
  group_by(which column should go here?) %>% # we must group data because we need maximums of each indice
  mutate(max_val = ifelse(nameofcolumn==max(nameofcolumn), nameofcolumn, NA)) # ifelse(logical_test, what_if_yes, what_if_no)